...
#  if (file.exists("data/trrust_rawdata.txt") == FALSE){
...
#loadTRRUST <- function(){
#  fread("data/trrust_rawdata.txt", header=FALSE, stringsAsFactors = FALSE, showProgress = FALSE, data.table = FALSE)
#}
...
####
#Get nodes connected directly or indirectly to certain node/nodes
####

getConnectedNodes <- function(nodes,edges,nodeList){
  igraph_edges = graph_from_data_frame(edges, directed=FALSE)
  c = clusters(igraph_edges)
  sel_c = unique(c$membership[nodeList])
  sel_nodes = names(c$membership[c$membership %in% sel_c])
  return(nodes[nodes$id %in% sel_nodes,])
}
...
visGraph <-function(nodes,edges){
  visNetwork(nodes, edges, width = "100%") %>% visEdges(arrows = trrust_network_mode[2], font = list(align = "middle", size = "9"), smooth = FALSE) %>%
